...
sub squid_get_values {
    ...
    foreach my $ipaddr (@ipaddrs) {
        ...
        # quick hack to fix NoForwardProxyPorts error
        # see: https://wiki.squid-cache.org/KnowledgeBase/NoForwardProxyPorts
        $output .= "http_port 127.0.0.1:$n_port\n";
...
if ($update_webproxy) {
    ...
    if ($squidguard_enabled) {
        ...
        system("sudo -u proxy squidGuard -C all > /dev/null 2>&1");
    }
    squid_restart(1);
    exit 0;
}
